...
# Date: 4/9/2015
# Description: Queries an ONT and returns all ethernet services
#######################################################################
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   queryEthSvc(sessionID, gpon_type, gpon_fsan, ont) #query for ethSvc stuff (modify for ge, pots, etc)
...
def queryEthSvc(sessionID, gpon_type, gpon_fsan, ont):
   target_url = str(config.protocol)+'://'+str(config.host)+':'+str(config.port) +str(config.extension)
   xml_request = """
			...
			<get-config>
				...
				<filter type="subtree">
					<top>
						...
						<children>
								<type>EthSvc</type>
								<attr-list>admin descr tag-action bw-prof mcast-prof</attr-list>
						</children>
						</object>
					</top>
				</filter>
